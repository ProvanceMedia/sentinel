...
// both the per-service tools (services.ts) and the generic http_call tool. Plus an SSRF
// guard — these fetches run UNCONTAINED on the host, so a target that resolves to a
...
export async function doFetch(url: URL, init: RequestInit, timeoutMs = 15_000): Promise<FetchResult> {
...
export async function ssrfCheck(hostname: string): Promise<string | null> {
